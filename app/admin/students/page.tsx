...
export default function AllStudents() {
    ...
    try {
      ...
      if (data.success) {
        ...
        const mappedStudents = data.data.map((student: any) => ({
          ...
          photo: student.photo || student.photoUrl || "", // Ensure photo field exists
        }))
        console.log("First student photo URL:", mappedStudents[0]?.photo || "No photo")
  ...
  const getStudentPhotoUrl = (student: Student) => {
    // If no photo URL is available, return the placeholder
    if (!student.photo && !student.photoUrl) {
      return "/placeholder-avatar.jpg";
    }
    
    // Use the photo URL directly
    const photoUrl = student.photo || student.photoUrl || "";
    
    // If it's an absolute URL or starts with /uploads/, return as is
    if (photoUrl.startsWith('http') || photoUrl.startsWith('data:') || photoUrl.startsWith('/uploads/')) {
      return photoUrl;
    }
    
    // For any other relative URLs, prepend the API base URL
