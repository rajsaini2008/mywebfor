...
export default function EditOnlineExamMarks({ params }: { params: { appId: string } }) {
  ...
  const fetchApplicationData = async () => {
    try {
      ...
      if (data.success) {
        ...
        if (data.data.atcId !== user?._id) {
          console.error("Application doesn't belong to this ATC");
          toast({
            title: "Access Denied",
            description: "You don't have permission to view this application",
            variant: "destructive",
          });
          router.push('/atc/exams/online-marks');
          return;
        }
        
        // Verify this is an online exam
        if (data.data.paperType !== 'online') {
          console.error("This is not an online exam application");
          toast({
            title: "Wrong Exam Type",
            description: "This is not an online exam application",
            variant: "destructive",
          });
          router.push('/atc/exams/online-marks');
          return;
  ...
  const saveResults = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "An error occurred while saving the marks",
        variant: "destructive",
      });
    }
  }
  ...
  return (
    <div className="px-4 py-6">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Online Exam Marks Update</h1>
      </div>
          ...
          {subjects.length > 0 && (
            <div className="flex justify-between items-center mt-6">
              ...
              <div className="space-x-2">
                <Button
                  variant="outline"
                  onClick={() => router.push('/atc/exams/online-marks')}
                >
                  Cancel
                </Button>
                <Button
