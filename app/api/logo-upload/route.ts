...
/**
 * API endpoint for handling logo image uploads
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uniqueFileName = `logo-${Date.now()}-${fileName}`;
    ...
    return NextResponse.json(
      { success: false, message: error.message || 'Failed to upload logo image' },
      { status: 500 }
    );
  }
} 